...
- This is distinct from request-local logs and should not depend on log scraping alone.
...
| Vector store collection registry | router runtime, `src/semantic-router/pkg/vectorstore/manager.go` | Pluggable `MetadataRegistry`: in-memory (default) or Postgres | `shared_durable_workflow_state` when `metadata_store: postgres`; otherwise ephemeral | With `metadata_store: memory`, collection metadata disappears on restart even if backend collections remain. With `postgres`, metadata survives restarts. | Set `vector_store.metadata_store: postgres` and provide `metadata_postgres` connection config for durable registry. A warning is emitted when `metadata_store` is `memory` with a durable backend (Milvus/Valkey). | Postgres registry tables: `vector_store_registry`, `file_registry`. Loaded on startup via `LoadFromRegistry`. |
| Vector store file registry | router runtime, `src/semantic-router/pkg/vectorstore/filestore.go` | File bytes on local disk; file metadata backed by `MetadataRegistry` | Mixed `restart_safe_local_state` and `shared_durable_workflow_state` | With `metadata_store: memory`, files may remain on disk while metadata vanishes on restart. With `postgres`, file metadata is durable. | Set `vector_store.metadata_store: postgres` for restart-safe file metadata. File bytes remain on local disk. | Shares the same `MetadataRegistry` as the collection registry. |
| Startup readiness and model download progress | router runtime, `src/semantic-router/pkg/startupstatus/status.go` | JSON file in config dir if writable, otherwise temp-owned path | `audit_analytics_telemetry` implemented as local temp/file convention | Dashboard may lose progress history or read from temp fallback paths | Status semantics depend on writable paths and log interpretation instead of typed lifecycle records | Replace temp/file guessing with typed status records or events owned by the server control plane. Keep local file output only as a local-dev adapter. |
